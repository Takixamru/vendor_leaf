...
if [ "$1" ]; then
    ...
    DEVICE=$(echo $TARGET_PRODUCT | sed 's/leaf_//g')
    if [ -f $file_path ]; then
        # only generate for official builds. unless forced with 'export FORCE_JSON=1'
        if [[ $file_name == *"UNOFFICIAL"* ]] || [[ $FORCE_JSON == 1 ]]; then
            ...
            build_type=$(grep ro\.leaf\.buildtype ./out/target/product/$DEVICE/system/build.prop | cut -d= -f2);
            base_version=$(grep ro\.leaf\.base\.version ./out/target/product/$DEVICE/system/build.prop | cut -d= -f2);
            link="https://sourceforge.net/projects/roms/files/Stone/leaf/${file_name}/download"
            echo "{" > $file_path.json
